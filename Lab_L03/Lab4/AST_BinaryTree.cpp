...
public:
    ...
    void BFS() {
        ...
        queue<Node*> q; // Use a queue to store nodes at each level
        q.push(root);
        ...
        while (!q.empty()) {
            ...
            if (current->pLeft != nullptr) q.push(current->pLeft); // Add left child to the queue
            if (current->pRight != nullptr) q.push(current->pRight); // Add right child to the queue
        }
        cout << endl;
    }
    // STUDENT ANSWER END
