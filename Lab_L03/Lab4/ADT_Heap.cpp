...
template<class T>
bool Heap<T>::isEmpty() {
    return count == 0; // Return true if the heap is empty, false otherwise
}
...
T Heap<T>::peek() {
    ...
    return elements[0]; // The root of the heap is the maximum element
}
...
void Heap<T>::remove(T item) {
    int index = getItem(item); // Find the index of the item
    if (index == -1) return;  // If the item is not found, do nothing
...
template<class T>
void Heap<T>::clear() {
    delete[] elements; // Free the memory of the current array
    capacity = 10;     // Reset the capacity to the initial value
    count = 0;         // Reset the count
    elements = new T[capacity]; // Allocate a new array with the initial capacity
}
int main() {
    Heap<int> heap;
